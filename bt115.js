...
(function () {
  ...
  function findLinksAndCreateButtons() {
    ...
    if (blockcode) {
      ...
      codes.forEach((code) => {
        ...
        const ed2kLinks = textContent.match(
          /ed2k:\/\/\|file\|.*?\|[0-9]+\|[A-Fa-f0-9]+\|\//g
        );
        if (magnetLinks) {
  ...
  // 添加右键点击监听器并执行 handleLinks
  document.addEventListener("contextmenu", function (event) {
    event.preventDefault();
    const selectedText = window.getSelection().toString();
    if (selectedText) {
      handleLinks([selectedText], addBotTorrents);
    }
  });
  // 添加bot离线下载任务
        ...
        GM_xmlhttpRequest({
          ...
          onload: (responseDetails) => {
            ...
            if (responseDetails.status === 200) {
              return responseDetails.responseText.includes("成功")
                ? resolve("添加" + responseDetails.responseText)
                : reject(responseDetails.responseText);
            }
      ...
      const fetchUserIDAndAddTorrents = () => {
        GM_xmlhttpRequest({
          ...
          onload: (responseDetails) => {
            ...
            const userID =
              resData.data && resData.data[0] ? resData.data[0].user_id : null;
            if (!userID) {
      ...
      const addTorrents = (userID) => {
        GM_xmlhttpRequest({
          ...
          onload: (responseDetails) => {
            ...
            let encodedUrls = falseUrl
              ? `url=${encodeURIComponent(urls[0])}`
              : urls
                  .map(
                    (url, index) => `url[${index}]=${encodeURIComponent(url)}`
                  )
                  .join("&");
              ...
              onload: (res) => {
                ...
                if (!falseUrl) {
                  if (resData.state === false)
                    return reject(resData.error_msg || "添加任务失败");
  ...
  function customNotify(message) {
    ...
    // 创建 append_parent 元素如果它不存在
    if (!appendParent) {
      appendParent = document.createElement("div");
      appendParent.id = "append_parent";
      document.body.appendChild(appendParent);
    }
    ...
    if (!ntcwin) {
      ...
      ntcwin.style.cssText =
        "position: fixed; z-index: 501; left: 50%; top: 117px; display: none; transform: translateX(-50%);";
      ntcwin.innerHTML = `
  ...
  function showCookie() {
    ...
    GM_cookie.list({ domain: ".115.com" }, function (cookieInfos, error) {
      if (!error) {
        ...
        alert(
          "获取cookie失败，请检查是否支持GM_cookie函数（目前只有beta版支持）"
        );
      }
    });
  }
  ...
  function handleCookieLogin(requireCookieMap, validDuration) {
    requireCookieMap.forEach((value, key) => {
      GM_cookie.delete({ name: key }, function (error) {
        if (error) {
          alert(
            `清除Cookie：[${key}]失败！请检查是否支持GM_cookie函数（目前只有beta版支持）`
          );
        }
      });
      GM_cookie.set(
        {
          ...
          httpOnly: false,
          expirationDate:
            Math.floor(Date.now() / 1000) + 60 * 60 * 24 * validDuration, // Expires in 30 days
        },
        function (error) {
          if (error) {
            alert(
              `设置Cookie：[${key}]失败，值为：[${value}]！请检查是否支持GM_cookie函数（目前只有beta版支持）`
            );
          }
        }
      );
  ...
  function showCookieLoginInputDialog() {
    ...
    let requireCookieMap = getRequireFieldFromCookie(
      requireCookieNames,
      inputCookie
    );
    if (requireCookieMap.size != requireCookieNames.length) {
      alert(
        `输入的Cookie需包含[${requireCookieNames.join(",")}]，请重新输入！`
      );
      return;
    }
    let defaultValidDuration = 30;
    let inputValidDuration = prompt(
      "请输入Cookie有效天数：",
  ...
  function initCookieLoginButton() {
    let loginFooter = document.querySelector(
